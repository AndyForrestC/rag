...
class TestRAGIntegration:
    ...
    def test_document_ingestion(self):
        ...
        response = requests.post(
            f"{INGESTION_BASE_URL}/api/ingestion/upload", files=files, timeout=30
        )
    ...
    def test_chat_query(self):
        ...
        response = requests.post(
            f"{INFERENCE_BASE_URL}/api/chat", json=payload, timeout=30
        )
    ...
    def test_end_to_end_rag_pipeline(self):
        ...
        chat_response = requests.post(
            f"{INFERENCE_BASE_URL}/api/chat", json=query_payload, timeout=45
        )
    ...
    def test_concurrent_requests(self):
        ...
        def make_chat_request(query_id):
            payload = {
                "messages": [
                    {"role": "user", "content": f"Test query {query_id}: What is AI?"}
                ]
            }
    ...
    def test_error_handling(self):
        ...
        response = requests.post(
            f"{INFERENCE_BASE_URL}/api/chat", json=invalid_payload, timeout=10
        )
        ...
        response = requests.post(
            f"{INGESTION_BASE_URL}/api/ingestion/upload", files=files, timeout=10
        )
